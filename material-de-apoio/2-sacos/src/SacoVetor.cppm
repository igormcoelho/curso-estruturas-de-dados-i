module;
export module saco_vetor;
import saco_tad;

export struct IteradorVetor1 {
  typedef char Tipo;
  Tipo* elemento;   // elemento atual
  Tipo* sentinela;  // elemento sentinela "final"
  Tipo atual();     // inglês 'current'
...
char IteradorVetor1::atual() { return *this->elemento; }

bool IteradorVetor1::terminou() { return this->elemento == this->sentinela; }

void IteradorVetor1::proximo() { this->elemento++; }

constexpr int MAX_N = 5;
export struct SacoVetor1 {
  typedef char Tipo;
  ...
  // range-based loop
  ItTipo begin() { return itera(); }
  // sentinela marcador de fim
  ItTipo end() { return IteradorVetor1{.elemento = elementos + N}; }
};

void SacoVetor1::cria() { this->N = 0; }

void SacoVetor1::libera() { this->N = 0; }
...
IteradorVetor1 SacoVetor1::itera() {
  IteradorVetor1 it{.elemento = this->elementos,
...
IteradorVetor1 SacoVetor1::busca(char dado) {
  for (int i = 0; i < N; i++)
    if (elementos[i] == dado)
      return IteradorVetor1{.elemento = this->elementos + i,
                            .sentinela = this->elementos + N};
  return IteradorVetor1{.elemento = this->elementos + N,
                        .sentinela = this->elementos + N};
}
...
export IteradorVetor1 buscarec(IteradorVetor1 it, char dado) {
  if (it.atual() == dado)
    return it;
  else {
    it.proximo();
