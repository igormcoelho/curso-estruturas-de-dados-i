module;
export module saco_lista_encadeada;
import saco_tad;
...
export struct IteradorNoEnc1 {
  typedef char Tipo;
  NoEnc1* no;       // colocação atual
  ...
  //
  bool operator!=(const IteradorNoEnc1& other) { return no != other.no; }
  //
  void operator++() { proximo(); }
};

char IteradorNoEnc1::atual() { return this->no->dado; }

bool IteradorNoEnc1::terminou() { return this->no == 0; }

void IteradorNoEnc1::proximo() { this->no = this->no->proximo; }

export struct ListaEnc1 {
  typedef char Tipo;
  ...
  //
  ItTipo end() { return IteradorNoEnc1{.no = 0}; }
};
...
export IteradorNoEnc1 buscarec(IteradorNoEnc1 it, char dado) {
